...
function formBuilderHelpersFn(opts, formBuilder) {
  ...
  _helpers.classNames = function (field, previewData) {
    var noFormControl = ['checkbox', 'checkbox-group', 'radio-group'],
        blockElements = ['header', 'paragraph', 'button', 'date-input'],
        i = void 0;
...
(function ($) {
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var defaults = {
      ...
      controlOrder: ['autocomplete', 'checkbox', 'checkbox-group', 'date', 'file', 'radio-group', 'select', 'text', 'textarea'],
      dataType: 'xml',
      /**
    ...
    var viewData = _helpers.markup('button', viewDataText, {
      id: frmbID + '-view-data',
      type: 'button',
      className: 'view-data btn btn-success'
    }),
    ...
    xmlButton.click(function (e) {
      ...
      var xml = _helpers.htmlEncode(elem.val()),
          ...
          /*$.ajax({
            ...
            data: {data:code.innerHTML},
            type: 'post',
            success:function(data) {
              alert("Posted");
            }
          });*/
      //$('.xml').inner(code.innerHTML);
      //document.getElementById('custom-rendered-fields').innerText = code.innerHTML;
      document.getElementById('custom-rendered-fields').value = code.innerHTML;
      //var myform = document.getElementById('fields-form');
      _helpers.confirm("Saved fields cannot be deleted. Are you sure you want to save?",  function()
      {
        document.getElementById('fields-form').submit();
      });
      /*if (confirm('Saved fields cannot be deleted. Are you sure you want to save?')) {
        // Save it!
        document.getElementById('fields-form').submit();
      } else {
        // Do nothing!
