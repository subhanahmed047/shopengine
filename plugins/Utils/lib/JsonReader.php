 ...
 */
	public function read($key) {
		if (strpos($key, '..') !== false) {
			throw new Exception(__('Cannot load configuration files with ../ in them.'));
		}
		...
		if (!is_file($file)) {
			if (!is_file(substr($file, 0, -4))) {
				throw new Exception(__('Could not load configuration files: %s or %s', $file, substr($file, 0, -4)));
			}
		}
